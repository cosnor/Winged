...
import { getZones, getZoneDistribution } from '../../../../services/mapsService';
...
export default function ZonesScreen() {
  ...
  const [selectedZoneCoords, setSelectedZoneCoords] = useState<{ lat: number; lon: number } | null>(null);
    ...
    if (value !== null && value !== 0) {
      const fetchZoneDistribution = async () => {
        try {
          ...
          const zoneName = zones.find(z => z.id === value)?.nombre || '';
          ...
          if (response.species_distributions) {
            setSelectedSpecies(response.species_distributions);
            setSelectedZoneName(response.zone);
            setSelectedZoneCoords({
          ...
          Alert.alert('Error', 'No se pudo obtener la distribución de especies para esta zona');
        } finally {
          setLoadingDistribution(false);
        }
      };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.mapContainer}>
        <Text style={styles.subtitle}>Selecciona una zona para ver la distribución de especies</Text>
        <MapView
          style={styles.map}
          ...
          initialRegion={userRegion || BARRANQUILLA_REGION}
            ...
            >
          {/* Render selected species distributions */}
        ...
        {(loadingZones || loadingDistribution) && (
      ...
      {selectedZoneName && selectedSpecies.length > 0 && (
        <View>
          <Text style={[styles.label, { marginTop: 16 }]}>Especies en {selectedZoneName}:</Text>
          <ScrollView style={styles.speciesListContainer}>
            {selectedSpecies.map((species, idx) => (
              <View key={idx} style={styles.speciesItem}>
                ...
                <Text style={styles.probabilityText}>
                  Probabilidad máxima: {(species.max_probability * 100).toFixed(1)}%
                </Text>
                <Text style={styles.helpText}>
                  {species.areas.length} área(s) de distribución
                </Text>
              </View>
            ))}
          </ScrollView>
      ...
      {selectedZoneName && selectedSpecies.length === 0 && !loadingDistribution && (
        <View style={styles.constructionContainer}>
          <Text style={styles.constructionText}>📊 No se encontraron especies en esta zona.</Text>
        </View>
      )}
      ...
      {loadingDistribution && (
        <View style={styles.constructionContainer}>
          ...
          <Text style={[styles.constructionText, { marginTop: 8 }]}>Cargando distribución...</Text>
        </View>
      )}
  ...
  function handlePolygonPress(sp: any, area: any) {
    Alert.alert(
      sp.species,
      `Probabilidad: ${(area.probability * 100).toFixed(1)}%`,
      [{ text: 'OK', onPress: () => {} }]
    );
  }
}
